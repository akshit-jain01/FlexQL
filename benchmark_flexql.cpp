...
static bool run_exec(flexql_db *db, const string &sql, const string &label) {
    char *errMsg = nullptr;
    ...
    if (rc != FLEXQL_OK) {
        std::cout << "[FAIL] " << label << " -> " << (errMsg ? errMsg : "unknown error") << "\n";
        if (errMsg) {
            flexql_free(errMsg);
        }
        return false;
    ...
    std::cout << "[PASS] " << label << " (" << elapsed << " ms)\n";
    return true;
}

static bool run_query(flexql_db *db, const string &sql, const string &label) {
    QueryStats stats;
    char *errMsg = nullptr;
    auto start = high_resolution_clock::now();
    ...
    std::cout << "[PASS] " << label << " | rows=" << stats.rows << " | " << elapsed << " ms\n";
    return true;
}

static bool query_rows(flexql_db *db, const string &sql, vector<string> &out_rows) {
    struct Collector {
        vector<string> rows;
    } collector;
    ...
    if (rc != FLEXQL_OK) {
        std::cout << "[FAIL] " << sql << " -> " << (errMsg ? errMsg : "unknown error") << "\n";
        if (errMsg) {
            flexql_free(errMsg);
        }
        return false;
...
static bool expect_query_failure(flexql_db *db, const string &sql, const string &label) {
    char *errMsg = nullptr;
    int rc = flexql_exec(db, sql.c_str(), nullptr, nullptr, &errMsg);
    if (rc == FLEXQL_OK) {
        std::cout << "[FAIL] " << label << " (expected failure, got success)\n";
        return false;
    }
    if (errMsg) {
        flexql_free(errMsg);
...
static bool assert_row_count(const string &label, const vector<string> &rows, size_t expected_count) {
    ...
    std::cout << "[FAIL] " << label << " (expected " << expected_count << ", got " << rows.size() << ")\n";
    return false;
}

static bool run_data_level_unit_tests(flexql_db *db) {
    std::cout << "\n\n[[ Running Unit Tests ]]\n\n";
...
static bool run_insert_benchmark(flexql_db *db, long long target_rows) {
    if (!run_exec(
            db,
